 ...
 */
fun mediatorLiveDataBuilder(vararg liveData: LiveData<*>): MediatorLiveData<*> {
    val mediator: MediatorLiveData<Any> = MediatorLiveData()
    liveData.forEach {
        mediator.addSource(it) { mediator.value = it}
    }
    return mediator
}